...
BP net;
vector<pair<vector<double>, vector<double>>> train_data, test_data;
int main() {
    /* net.init({196, 300, 300, 10}); */
    ifstream fin;
    fin.open("MNIST.txt", ios::in);
    fin >> net;
    fin.close();
    ...
    for (int i = 0; i < _images.size(); i++) {
        vector<double> out = vector<double>(10, 0.1);
        out[_labels[i]] = 0.9;
        test_data.push_back({pool(_images[i]), out});
        /* net.run(pool(_images[i])); */
        /* cnt+=net.chk(out); */
    };
    /* cout<<cnt<<endl; */
    for (int i = 0; i < images.size(); i++) {
        vector<double> out = vector<double>(10, 0.1);
        out[labels[i]] = 0.9;
        train_data.push_back({pool(images[i]), out});
    };

    net.train_all(train_data, test_data, 0.005, 1e5, 1000);
    ofstream fout;
    fout.open("MNIST.txt", ios::out);
    fout << net;
    fout.close();
    return 0;
}
